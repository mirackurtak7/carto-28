...
def generate_launch_description():
    ...
    # Tortoisebot Navigation Launch
    navigation_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource([os.path.join(
            get_package_share_directory('ros2_slam_auto_navigation'), 'launch', 'navigation.launch.py'
        )])
    )

    # Tortoisebot SLAM Launch
    slam_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource([os.path.join(
            get_package_share_directory('tortoisebot_slam'), 'launch', 'cartographer.launch.py'
        )])
    )
